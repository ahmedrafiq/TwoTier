...
/*function get_campaign_customers($campaign_list_array, $customers_list_array, $campaign_customers_array, $campaign_id, $show_master_campaign=false)
{
	if(isset($campaign_list_array['campaigns']['campaign'][0]))
	{
		if($show_master_campaign == false)
		{
			...
			foreach($campaign_list_array['campaigns']['campaign'] as $single_camp) 
			{
				...
				{
					$key = $single_camp['id'];
					$campaign_customers_array[$key] = $single_camp;
					if($c==0 and $campaign_id=='')
					{
						$campaign_id = $key;
					}
		...
		{
			$key = $campaign_list_array['campaigns']['campaign']['id'];
			$campaign_customers_array[$key] = $campaign_list_array['campaigns']['campaign'];
			$campaign_id = $key;
		}
	}
				
	if($customers_list_array['@attributes']['status'] == 'success')
	{
		if(isset($customers_list_array['customer'][0]))
		{
			$i=0;
			foreach($customers_list_array['customer'] as $single_customer)
			{
				$cusomter_code = $single_customer['code'];
				$card_number = $single_customer['card_number'];
				if(isset($single_customer['campaigns']['campaign'][0]))
				{
					foreach($single_customer['campaigns']['campaign'] as $single_campaign)
					{
						$key = $single_campaign['id'];
						if(isset($campaign_customers_array[$key]))
						{
							$campaign_customers_array[$key]['customers'][$i]['code'] = $single_customer['code'];
							$campaign_customers_array[$key]['customers'][$i]['card_number'] = $single_customer['card_number'];
							$campaign_customers_array[$key]['customers'][$i]['first_name'] = $single_customer['first_name'];
							$campaign_customers_array[$key]['customers'][$i]['last_name'] = $single_customer['last_name'];	
							
							$cust_transac = json_decode(json_encode(get_customer_balance($key, $cusomter_code, $card_number)), true);
							
							if($cust_transac['@attributes']['status'] == 'success')
							{
								if(isset($cust_transac['campaign']['customer']['transactions']['transaction'][0]))
								{
									foreach($cust_transac['campaign']['customer']['transactions']['transaction'] as $single_transaction)
									{
										if($single_transaction['amount'] >= 0 and $single_transaction['service_product'] != 'Activation')
										{
											$campaign_customers_array[$key]['customers'][$i]['transactions'][] = $single_transaction;
										}
									}
								} else {
									if($cust_transac['campaign']['customer']['transactions']['transaction']['amount'] >= 0 and $cust_transac['campaign']['customer']['transactions']['transaction']['service_product'] != 'Activation')
									{
										$campaign_customers_array[$key]['customers'][$i]['transactions'][0] = $cust_transac['campaign']['customer']['transactions']['transaction'];
									}
								}
							} 
						}
					...
					$key = $single_customer['campaigns']['campaign']['id'];
					if(isset($campaign_customers_array[$key]))
					{
						$campaign_customers_array[$key]['customers'][$i]['code'] = $single_customer['code'];
						$campaign_customers_array[$key]['customers'][$i]['card_number'] = $single_customer['card_number'];
			...
			$cusomter_code = $customers_list_array['customer']['code'];
			$card_number = $customers_list_array['customer']['card_number'];
			if(isset($customers_list_array['customer']['campaigns']['campaign'][0]))
			{
				foreach($customers_list_array['customer']['campaigns']['campaign'] as $single_campaign)
				{
					$key = $single_campaign['id'];
					if(isset($campaign_customers_array[$key]))
					{		
						$campaign_customers_array[$key]['customers'][0]['code'] = $customers_list_array['customer']['code'];
						$campaign_customers_array[$key]['customers'][0]['card_number'] = $customers_list_array['customer']['card_number'];
						$campaign_customers_array[$key]['customers'][0]['first_name'] = $customers_list_array['customer']['first_name'];
						$campaign_customers_array[$key]['customers'][0]['last_name'] = $customers_list_array['customer']['last_name'];
						...
						if($cust_transac['@attributes']['status'] == 'success')
						{
							if(isset($cust_transac['campaign']['customer']['transactions']['transaction'][0]))
							{
								foreach($cust_transac['campaign']['customer']['transactions']['transaction'] as $single_transaction)
								{
									if($single_transaction['amount'] >= 0 and $single_transaction['service_product'] != 'Activation')
									{
										$campaign_customers_array[$key]['customers'][0]['transactions'][] = $single_transaction;
									}
								}
							} else {
								if($cust_transac['campaign']['customer']['transactions']['transaction']['amount'] >= 0 and $cust_transac['campaign']['customer']['transactions']['transaction']['service_product'] != 'Activation')
								{
									$campaign_customers_array[$key]['customers'][0]['transactions'][0] = $cust_transac['campaign']['customer']['transactions']['transaction'];
								}
							}
						}
					}
				...
				$key = $customers_list_array['customer']['campaigns']['campaign']['id'];
				if(isset($campaign_customers_array[$key]))
				{
					$campaign_customers_array[$key]['customers'][0]['code'] = $customers_list_array['customer']['code'];
					$campaign_customers_array[$key]['customers'][0]['card_number'] = $customers_list_array['customer']['card_number'];
	...
	return array($campaign_customers_array, $campaign_id);
}
# =================

function campaign_customers_array_build($campaign_customers_array, $default_percentage, $contribution_percentage, $start_date='', $show_master_campaign=false)
{
	...
	if($start_date == '')
	{
		...
		foreach($campaign_customers_array as $cid=>$curr_campaign)
		{
			...
			{
				foreach($curr_customer['transactions'] as $curr_transaction)
				{
					$amount = $curr_transaction['orig_amount'];
					if($show_master_campaign)
					{
						$amount = $curr_transaction['amount'];
					}
					$total_spent = $total_spent + $curr_transaction['orig_amount'];
					$transactions_rows .= '<tr>';
						$transactions_rows .= '<td>'.$curr_customer['card_number'].'</td>';
						$transactions_rows .= '<td>'.$curr_customer['first_name'].'</td>';
						$transactions_rows .= '<td>$'.$amount.'</td>';
						$transactions_rows .= '<td>'.$curr_transaction['date'].'</td>';
					$transactions_rows .= '</tr>';
			...
			$contrib_perc = isset($contribution_percentage[$cid]) ? $contribution_percentage[$cid] : $default_percentage ;
			$campaign_customers_array[$cid]['total_amount_spent'] = $total_spent;
			$campaign_customers_array[$cid]['invoice_amount'] = ($total_spent * $contrib_perc) / 100;
			$campaign_customers_array[$cid]['transactions_rows'] = $transactions_rows;
			
			$overall_spent = $overall_spent + $total_spent;
			$overall_invoice_amount = $overall_invoice_amount + $campaign_customers_array[$cid]['invoice_amount'];
		}
		$campaign_customers_array['overall_spent'] = $overall_spent;
		$campaign_customers_array['overall_invoice_amount'] = $overall_invoice_amount;
	} else {
		$overall_spent = 0;
		$overall_invoice_amount = 0;
		foreach($campaign_customers_array as $cid=>$curr_campaign)
		{
			...
			foreach($curr_campaign['customers'] as $curr_customer)
			{
				foreach($curr_customer['transactions'] as $curr_transaction)
				{
					if(strtotime($curr_transaction['date']) > strtotime($start_date))
					{
						$amount = $curr_transaction['orig_amount'];
						if($show_master_campaign)
						{
	...
	return $campaign_customers_array;
}*/
?>
